...
* `nvda.should_log("text")` waits for a log record. It searches the whole log since the test began,
  not only records written after the last action.
...
Long lists show ten items. Run with `--nvda-verbose` to see all. For terminal output without colour,
run `pytest --color=no`.
