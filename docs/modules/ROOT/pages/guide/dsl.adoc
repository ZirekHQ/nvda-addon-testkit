...
`relaunch`, `restart_nvda`, `install_addon`, `remove_addon`, `should_have_addon`,
...
`wait_until_idle`, `reset`, `restart_harness` and `keys.*` are not guarded and still block. Needs
`--nvda-allow-eval`.
