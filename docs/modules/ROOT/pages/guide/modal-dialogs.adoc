...
Both need `--nvda-allow-eval`, the same as `eval()`/`exec()` — `exec_nowait()`
runs arbitrary code inside NVDA just like `exec()` does; `simulate_modal()`
itself doesn't, but only makes sense paired with a scenario that does.

Pair the two: `exec_nowait()` to open the dialog, `simulate_modal()` to
close it. Calling `exec()` instead of `exec_nowait()` to open the dialog
defeats the point — `simulate_modal()`'s call would never even be
dispatched.
